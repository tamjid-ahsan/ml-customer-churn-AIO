...
# Imports
# -------------------------------------------------------------------------
data_scaled = joblib.load('./app/model/scaled_data.joblib')

data = joblib.load('./app/model/unscaled_data.joblib')
data_cluster = data.groupby('Clusters').count()['target'].sort_index(
    ascending=False).reset_index()
preprocessor = joblib.load('./app/model/preprocessor.joblib')
nume_col = joblib.load('./app/model/nume_col.joblib')
cate_col = joblib.load('./app/model/cate_col.joblib')
# prediction_model = joblib.load(
#     './app/model/xgb_clf_churn_prediction_all_data.joblib')
# kmeans_model = joblib.load('./app/model/kmeans_segmentation_model.joblib')
...
app.layout = dbc.Container([
    ...
    dbc.Row(
        dbc.Col([
            html.I("Please Input Features: [press `ENTER` or `Predict` after inputting]"),
            html.Br(),
...
def update_output(Gender, Dependent_count, Education_Level, Marital_Status,
    ...
    if sum(new_data.iloc[0].isnull())==0:
        ...
        return f'Client is predicted to be {prediction_label}ing.'
    else:
